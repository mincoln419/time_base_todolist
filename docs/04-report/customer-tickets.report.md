...
**Added**
- "캘린더" 탭 신설 — 월 단위 그리드에 `desired_date`가 설정된 티켓을 날짜별로 표시 (◀/▶ 월 이동, 오늘 날짜 강조)
- `GET /api/tickets` — 전 고객사 티켓을 `customer_name` 조인해 반환 (캘린더 데이터 소스)
- 캘린더의 티켓을 더블클릭하면 "고객사 티켓" 탭으로 딥링크 — 해당 고객사가 자동 선택되고 티켓 행이 2초간 하이라이트됨

**Verified**
- curl로 `GET /api/tickets`의 `customer_name` 조인 결과 확인
- Playwright E2E로 희망 일자가 있는 티켓이 캘린더의 올바른 날짜 칸에 표시됨을 확인, 더블클릭 시 탭 전환·고객사 자동 선택·행 하이라이트까지 실제 브라우저에서 확인, 콘솔 에러 0건
- 실사용 데이터(북미SDS 등 기존 고객사/티켓)에 영향 없음을 확인 (E2E는 임시 고객사로 수행 후 정리)
