...
- [ ] `daily_notes` 테이블 신설 (date, keyword, category, related_keywords, item, content, created_at, updated_at)
- [ ] `GET/POST/PUT/DELETE /api/daily-notes` API 신설
- [ ] "데일리노트" 신규 메뉴 탭 등록 (`App.jsx` TABS)
- [ ] 입력 폼: 키워드 · 카테고리 · 연관 키워드 · 항목(텍스트 입력) + 본문(마크다운 텍스트에어리어, 최대 2000자, 글자 수 카운터)
...
- [ ] 마인드맵 뷰 — 노트를 노드로, 같은 카테고리 또는 연관 키워드로 연결된 노트끼리 선으로 연결해 시각화, 노드 클릭 시 상세 열람
- [ ] 노트 수정/삭제
...
|----|-------------|----------|--------|
| FR-01 | `daily_notes` 테이블은 `id`(PK), `date`, `keyword`, `category`, `related_keywords`, `item`, `content`, `created_at`, `updated_at`을 저장한다 | High | Pending |
...
| FR-04 | `PUT /api/daily-notes/:id`는 해당 노트의 필드(키워드/카테고리/연관 키워드/항목/본문/날짜)를 갱신한다 | High | Pending |
...
| FR-08 | 입력 폼에는 키워드 · 카테고리 · 연관 키워드(쉼표 구분 다중 입력) · 항목 · 본문(마크다운) 필드가 모두 존재하며, 키워드를 제외한 나머지는 선택 입력이다 | High | Pending |
...
| FR-11 | 마인드맵 뷰는 각 노트를 노드로 그리고, 같은 `category` 값을 가지거나 서로의 `related_keywords`에 포함된 노트끼리 선으로 연결한다 | High | Pending |
...
| 연관 키워드 저장 형태 | (A) 별도 정규화 테이블(`daily_note_keywords`) / (B) `related_keywords` 컬럼에 쉼표 구분 문자열로 저장 | **(B)** | 사전 정의된 키워드 마스터 관리는 Out of Scope. 자유 텍스트 다중 값을 단순 문자열로 저장하는 것으로 충분하며, 마인드맵 연결 계산 시 클라이언트/서버에서 split+trim으로 처리 |
...
└── components/DailyNote/
    ...
    ├── DailyNoteForm.jsx       # 키워드/카테고리/연관 키워드/항목/본문(마크다운) 입력 폼
