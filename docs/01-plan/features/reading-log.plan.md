...
- **책마다 하루 목표 페이지 설정** (2026-09-24 amendment) — 도서관 대출(최대 21일) 책은 반납일 안에 끝나도록 목표를 올릴 수 있어야 한다.
...
| FR-16 | 책 등록/수정 폼에서 하루 목표 페이지(1 이상 정수, 기본 10)를 입력할 수 있고, "대출 21일에 맞추기" 버튼은 반납일(시작일 + 20일)까지 남은 날로 남은 페이지를 나눠 하루 목표를 채운다(10 미만이면 10 유지). 기존 책은 10페이지 전제였으므로 `daily_target = 10` (2026-09-24 추가) | High | Done |
