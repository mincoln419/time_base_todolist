...
| FR-18 | 체크리스트 행의 "메모" 버튼은 제목·내용(마크다운, 작성/미리보기)만 받는 모달을 열고, 저장 시 별도 컬렉션 없이 데일리노트(`dailyNotes`)에 `item`=제목, `source: 'reading'`, `book_id`로 저장한다. 태그(카테고리·키워드)는 저장 시점에 서버가 AI로 추출하고, 출처 태그 `오늘의독서`와 책 제목을 키워드에 함께 넣는다. AI 실패 시에도 메모는 이 두 키워드로 저장한다. 본문은 `> **오늘의 독서** · 《책 제목》 · 17 ~ 29p · 날짜` 머리말로 감싸 저장하고, 문서에도 `page_from`/`page_to`를 남겨 출처를 추적한다(그날 체크 전이면 "페이지 기록 전"). 메모는 오늘의 독서 화면에는 표시하지 않고 데일리노트에서 태그로 찾는다 (2026-09-24 추가/수정) | High | Done |
