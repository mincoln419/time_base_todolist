...
| FR-18 | 체크리스트 행의 "메모" 버튼은 제목·내용(마크다운, 작성/미리보기)만 받는 모달을 열고, 저장 시 별도 컬렉션 없이 데일리노트(`dailyNotes`)에 `item`=제목, `source: 'reading'`, `book_id`로 저장한다. 태그(카테고리·키워드)는 저장 시점에 서버가 AI로 추출하고 책 제목을 키워드에 함께 넣는다. AI 실패 시에도 메모는 책 제목 키워드로 저장한다. 해당 날짜의 메모는 행 아래에 마크다운으로 렌더링된다 (2026-09-24 추가) | High | Done |
